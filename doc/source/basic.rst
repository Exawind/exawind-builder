...
project that is being built. Lines 20-22 loads the core, machine-specific, and
project-specific functions. Lines 41-46 will have the appropriate :ref:`project
variables <ref-project-vars>` defined. Lines 1-37 should not be modified by the
user, they are automatically generated by ``exawind-builder``. It is recommended
that all user customizations be added within the block indicated by lines
48--50. User might want to customize :envvar:`install directory
<PROJECTNAME_INSTALL_DIR>` when building different variants (e.g., release,
debug, etc.). In the following sections, only the modifications that must be
entered between lines 48-50 are shown.
