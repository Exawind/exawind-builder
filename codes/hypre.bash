...
exawind_cmake_base ()
{
    ...
    if [ "${ENABLE_CUDA:-OFF}" = "ON" ] ; then
        ...
        if [ "${HYPRE_ENABLE_CUSOLVER:-ON}" = "ON" ] ; then
            echo "==> HYPRE: Enabling CUDA cusolver"
            cuda_args="${cuda_args} --enable-cusolver "
        else
            echo "==> HYPRE: Disabling CUDA cusolver"
            cuda_args="${cuda_args} --disable-cusolver "
        fi
